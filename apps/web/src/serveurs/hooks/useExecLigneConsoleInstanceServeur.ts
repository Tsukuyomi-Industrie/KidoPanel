...
import { posterExecInstanceServeurJeuxPasserelle } from "../../passerelle/serviceExecConteneurPasserelle.js";

/**
 * Accumule les sorties d’exécutions ponctuelles (`exec`) pour l’instance jeu,
 * distinctes du flux SSE Docker.
 */
export function useExecLigneConsoleInstanceServeur(params: {
  ...
  const envoyerLigneShell = useCallback(
    async (ligneBrute: string) => {
      ...
      try {
        const resultat = await posterExecInstanceServeurJeuxPasserelle(
          params.idInstanceJeux,
          { cmd: ["/bin/sh", "-c", ligne] },
        );
    ...
    [params.actif, params.idInstanceJeux],
  );
